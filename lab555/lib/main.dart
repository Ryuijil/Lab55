    ...
    setState(() {
      ...
      if(_formKey.currentState!.validate()  ) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Площадь вычислена, победа'), backgroundColor: Colors.blueAccent,)
        );
      }
    });
  }
                ...
                children: <Widget> [
                  length > 0 && height > 0 && result > 0 && (length * height) == result  ? Text('S = $length * $height = $result (мм2)',
                    style: const TextStyle(
                      ...
                      : length > 0 && height > 0 ? const Text('') : const Text('Введите параметры', style: TextStyle(
                      fontWeight: FontWeight.w500, fontSize: 26))
                ],
              ),
            )
